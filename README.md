...
Run Playwright tests against a running dev server (ensure `npm start` is running on http://localhost:3000):

- Install dev dependency: `npm install --save-dev @playwright/test`
- Install browser binaries: `npx playwright install`
- Run tests: `npm run test:ui` (headless) or `npm run test:ui:headed` (headed)
